...
impl ObjectImpl for OutputArea {
    ...
    fn constructed(&self) {
        ...
        drop_target.connect_drop(clone!(
            @weak self as this => @default-panic, move |dt, v, x, y| this.on_drop(dt, v, x, y)
        ));
        ...
        *bounds = Self::get_bounds(&outputs);
        for output in outputs.iter::<Output>().map(Result::unwrap) {
            if let Some(mode) = output.mode() {
                let max_x = i16::try_from(self.screen_max_width.get().saturating_sub(mode.width()))
                    .unwrap_or(i16::MAX);
                let max_y =
...
impl OutputArea {
    ...
    fn get_output_index_at(&self, x: f64, y: f64) -> Option<usize> {
        ...
        for (i, output) in self.outputs.borrow().iter::<Output>().map(Result::unwrap).enumerate() {
